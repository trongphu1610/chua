...
public class Registry {
    ...
    private <Data, TResource, Transcode> List<DecodePath<Data, TResource, Transcode>> getDecodePaths(@NonNull Class<Data> dataClass, @NonNull Class<TResource> resourceClass, @NonNull Class<Transcode> transcodeClass) {
        ...
        for (Class<TResource> registeredResourceClass : this.decoderRegistry.getResourceClasses(cls, resourceClass)) {
            for (Iterator<Class<Transcode>> it = this.transcoderRegistry.getTranscodeClasses(registeredResourceClass, transcodeClass).iterator(); it.hasNext(); it = it) {
                Class<Transcode> registeredTranscodeClass = it.next();
                Class<Transcode> cls2 = registeredTranscodeClass;
                decodePaths.add(new DecodePath<>(cls, registeredResourceClass, registeredTranscodeClass, this.decoderRegistry.getDecoders(cls, registeredResourceClass), this.transcoderRegistry.get(registeredResourceClass, registeredTranscodeClass), this.throwableListPool));
            }
        }
