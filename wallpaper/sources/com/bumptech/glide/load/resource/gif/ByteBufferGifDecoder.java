        ...
        try {
            ...
            if (header.getNumFrames() > 0) {
                if (header.getStatus() == 0) {
                    ...
                    int sampleSize = width;
                    int sampleSize2 = getSampleSize(header, sampleSize, height);
                    GifDecoder gifDecoder = this.gifDecoderFactory.build(this.provider, header, byteBuffer, sampleSize2);
                    gifDecoder.setDefaultBitmapConfig(config);
                    gifDecoder.advance();
                    Bitmap firstFrame = gifDecoder.getNextFrame();
                    if (firstFrame != null) {
                        int i = sampleSize2;
                        GifDrawableResource gifDrawableResource = new GifDrawableResource(new GifDrawable(this.context, gifDecoder, UnitTransformation.get(), sampleSize, height, firstFrame));
